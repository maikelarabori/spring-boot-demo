...
@DisplayName("DataElementController: integration test + doc")
public class DataElementControllerIntegrationTest {
  ...
  public void getDataElements() throws Exception {
    ...
    // When
    when(dataElementsGetEndPoint.consume(any(EndpointParams.class))).thenReturn(stubResponse);

    // Then
    this.mockMvc.perform(get("/api/data/elements")).andExpect(status().isFound())
